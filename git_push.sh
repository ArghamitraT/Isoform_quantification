...
for ext in "${EXTENSIONS[@]}"; do
    # find all files with this extension, excluding the .git/ directory
    mapfile -d '' FILES < <(
    ...
    if [[ ${#FILES[@]} -gt 0 ]]; then
        ...
        echo "  [+] Staged ${#FILES[@]} .${ext} file(s)"
        TOTAL_STAGED=$(( TOTAL_STAGED + ${#FILES[@]} ))
    else
