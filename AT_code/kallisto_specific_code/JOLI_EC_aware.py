...
from kallisto_ec_loader import load_kallisto_ec_with_counts
...
class Expec_Max:
    ...
    def __init__(
            ...
            use_kallisto_ec: bool = False,
            kallisto_dir: str = "",
            debug_max_ec: int | None = None,
        ...
        * quiet
        ...
        self.use_kallisto_ec = use_kallisto_ec
        self.kallisto_dir = kallisto_dir
        self.debug_max_ec = debug_max_ec
        ...
        if load:
            ...
            if self.use_kallisto_ec:
                self.initialize_model_from_kallisto_ec()
            else:
        ...
        for sample, theta_array in self.all_theta.items():
            ...
            total_reads = np.sum(self.all_n[sample])
            count_df["est_count"] = count_df["raw"] * total_reads
    ...
    def MAP(self):
        ...
        if "sample1" in self.all_theta:
            theta0 = self.all_theta["sample1"]
            print("theta_init sum:", np.sum(theta0))

            theta1, _ = self.update_theta_vectorized("sample1")
            print("theta after 1 EM step sum:", np.sum(theta1))
    ...
    def calculate_n_vectorized(self, sample_key):
        ...
        # weights per row
        weights = self.all_read_weights.get(sample_key)
        if weights is None:
            # default to 1 for every row (read-based pipeline)
            weights = np.ones_like(all_Phi, dtype=float)
        else:
        ...
        # Weighted Phi: each row's Z_ri * w_r
        filtered_Phi = all_Phi[valid_indices]
        filtered_weights = weights[valid_indices]
        weighted_Phi = filtered_Phi * filtered_weights
        ...
        np.add.at(n_values, filtered_indices, weighted_Phi) # Add weights to corresponding bins
    ...
    def initialize_model(self):
        ...
        for sample_key in self.all_read_dicts:
            ...
            self.all_read_weights[sample_key] = np.ones_like(compatibility_list, dtype=float)
    ...
    def initialize_model_from_kallisto_ec(self):
        """
        Initialize the model directly from Kallisto/BUS EC files:
          - transcripts.txt
          - count.ec.txt
          - count.mtx

        Uses ECs as pseudo-reads, with multiplicity from count.mtx.
        """
        start = time.time()
        ...
        (
            ...
            p_nm,
            read_weights,
        ) = load_kallisto_ec_with_counts(
            self.kallisto_dir,
            max_ec_nonzero=self.debug_max_ec,
        )

        print(f"[EC init] num rows (EC–transcript pairs): {len(all_read_names)}")
        print(f"[EC init] num unique ECs (pseudo-reads): {len(np.unique(all_read_names))}")
        print(f"[EC init] num unique transcripts in ECs: {len(np.unique(all_rnames))}")

        # We don't actually use read_dict / ref_len_dict in EC mode
        self.all_read_dicts[sample_key] = {}
        self.all_ref_len_dicts[sample_key] = {}

        self.all_Yri[sample_key] = compat_list
        self.all_read_iso_prob[sample_key] = p_nm

        # Map read & isoform names
        self.all_readIsoformMap[sample_key] = all_rnames
        self.all_readName[sample_key] = all_read_names

        # Store weights per row
        self.all_read_weights[sample_key] = read_weights

        # Initial theta from compatibilities
        theta_names, all_theta = self.calculate_theta_and_alpha_prime_0(
            all_rnames,
            compat_list,
        )
        self.all_theta[sample_key] = all_theta
        self.theta_names[sample_key] = theta_names

        # Track global set of isoforms
        all_unique_isoforms_set = set(theta_names)

        # Compute initial Z and n
        self.all_Phi_ri[sample_key] = self.calculate_Z(
            all_read_names,
            all_rnames,
            compat_list,
            p_nm,
        ...
        # Global isoform list + alpha
        self.all_unique_isoforms = np.array(sorted(all_unique_isoforms_set))
        self.all_alpha = self.assign_alpha()  # or assign_alpha_constant()

        end = time.time()
        interval = (end - start) / 60.0
        print(f"[EC init] time_initialize {interval:.3f} min")
        print("[EC init] Done. Starting EM...")
