...
def main():
    ...
    alpha_initial_list =  list(range(int(100e3), int(20000e3), int(20e3)))      # Initial alpha summation
    ...
    desired_corr=0.65
    spearman_corr_list, pearson_corr_list = [], []
...
#         # Create subplots
    ...
    ax1.plot(alpha_initial_list, spearman_corr_list, marker='o')
    ...
    ax2.plot(alpha_initial_list, pearson_corr_list, marker='o', color='orange')
    ...
    plt.tight_layout()
    # plt.grid()
    plt.savefig(f'figures/alphaInitialVSCorr{timestamp}.png')
    plt.show()
...
#     # Step 2: Generate a second sample that has the desired Spearman correlation with the first sample
#     sample2 = np.random.dirichlet(alpha)
...
#         start = time.time()
#         print(f"Generating reads for {read_name} day {day} {sample_name}...")
...
#     # Sample dictionaries of isoform abundances for two samples
#     sample_1 = isoform_proportion_samples.get('sample_1')
#     sample_2 = isoform_proportion_samples.get('sample_2')
...
#     # Calculate Spearman and Pearson correlations
#     spearman_corr, _ = scipy.stats.spearmanr(abundances_1, abundances_2)
#     pearson_corr, _ = scipy.stats.pearsonr(abundances_1, abundances_2)
...
#     isoform_number= int(150e3)     # How many isoforms per sample
#     alpha_initial_list =  list(range(int(100e3), int(2000e3), int(20e3)))    # Initial alpha summation
#     num_samples = 2            # Specify the number of samples you want to generate
...
#         spearman_corr, pearson_corr = Spearman_Pearson_corr_in_thetas(isoform_proportion_samples)
#         spearman_corr_list.append(spearman_corr)
#         pearson_corr_list.append(pearson_corr)
...
#     #     read_name = 'long'
#     #     corr_new = generate_read_counts_multiple_samples(isoform_proportion_samples, long_total_reads, isoform_lengths, simulation_dir, 
#     #         long_min_ambiguity, long_max_ambiguity,  long_min_read_length, long_avg_read_length, long_max_read_length, day, read_name)
        
#     #     file_path = os.path.join(simulation_dir, f"SRspCorr_{corr:.2f}_LRspCorr_{corr_new:.2f}_isoformAbundance_groundTruth.pkl")
#     #     # Save the read_dict as a pickle file
#     #     with open(file_path, 'wb') as f:
