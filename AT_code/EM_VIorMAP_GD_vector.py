...
class Expec_Max:
    ...
    def calculate_elbo(self, sample_key):
        ...
        rnames = self.all_readIsoformMap[sample_key]  # Array of isoform names for each read
    ...
    def update_alpha_prime(self, sample_idx):
        ...
        all_readIsoformMap = self.all_readIsoformMap[sample_idx]
        ...
        # Map `all_readIsoformMap` to indices using `np.searchsorted`
        rname_indices = np.searchsorted(sorted_unique_isoforms, all_readIsoformMap)
        valid_rname_mask = np.isin(all_readIsoformMap, sorted_unique_isoforms)
    ...
    def update_theta_vectorized(self, sample_key):
        ...
        if self.experiment_num == 4 or self.experiment_num == 5: # multi sample
            ...
            numerator = n_plus_alpha - 1

            # Calculate denominator: sum(numerator) - I
            denominator = np.sum(n_plus_alpha) - len(numerator)
